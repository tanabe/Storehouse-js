class Storehouse {
  ...
  static createKey = function(namespace, key) {
  ...
  static setItem = function(namespace, key, value, expire) {
    var item = {
      namespace: namespace,
      key: key,
      value: value
    ...
    if (expire) {
      ...
      localStorage.setItem(this.createKey(namespace, key), JSON.stringify(item));
    } else {
      sessionStorage.setItem(this.createKey(namespace, key), JSON.stringify(item));
    }
  };

  static deleteItem = function(namespace, key) {
    sessionStorage.removeItem(this.createKey(namespace, key));
    localStorage.removeItem(this.createKey(namespace, key));
  };

  static getInstance = function(namespace) {
    return {
      getItem: function(key) {
        return Storehouse.getItem(namespace, key);
      },

      setItem: function(key, value, expire) {
        Storehouse.setItem(namespace, key, value, expire);
      },
  ...
  static getMD5Hash = function(rawData) {
